...
- **自動取得:** GitHub Actions が土日 8–19 時 JST・30 分間隔で更新（8:00 は出馬表＋オッズ、以降は結果優先）。自宅の `fetcher` コンテナは起動しない
...
| 前走内容 | 着順をそのレースのペース（ラップ）・通過・枠の一般不利で読み替え | 展開逆行・枠不利の好走は着順以上 |
| 人気との乖離 | 実力評価とオッズの差 | 乖離大で加点 |
| パドック | **初期スコアに入れない** | 補足ノートのみ。日記へ残すのは可 |
...
- **データ方針:** Need×Auto を最優先。factors は当面 Derive（仮）。Manual（パドック・予想家など）は選別スコアに混ぜない。パドックは初期スコア対象外で確定。前走着順はラップと通過と枠の一般不利で読み替える（日内のトラックバイアス映像は未取得）
...
|--------------------|-------------|-----------|
| formSignal / 前走 | 複勝圏到達。着順はペース・通過で読み替え | **勝ち切れ**も同様に読み替え。パドックは使わない |
...
findDangerousFirstFavorite(race) =
  単勝1番人気が存在するとき:
    flagged ⇔
      ...
      OR 先行有利コースかつ脚質が差/追（runningStyle がある場合）
      OR formStats.lastDate があり raceDate まで 180 日以上
```
...
| 補助根拠 | 先行有利コース × 差し/追込。脚質未取得ならこの枝は発火しない |
| 週次1（2026-09-07） | 前走から **180日以上** の休み明け。`lastDate` が無い馬では発火しない |
| 軸・買い目 | **まだ切らない。** 印と freeze/evaluate の比較だけ |
| 検証 | フラグあり1人気 vs なし1人気の 1着率・複勝圏率 |

今後検証（危1にはまだ入れない）: 前走逃げ勝ち、特殊馬場勝ち→良馬場、枠代わり、牝馬限定→混合。
...
| 危険1人気 | 1番人気のうち、人気を除いた1着適性が弱い／先行有利コースの差し追込／半年以上の休み明け。印のみ。軸からは外さない |
