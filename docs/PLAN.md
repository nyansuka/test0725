...
- **公開Webスナップショット**（netkeiba 出馬表 HTML + オッズ API）を `src/data/snapshots/` に保存して表示
...
- **評価:** 関係馬が「人気薄でも 1〜3着に届きうるか」を数値化
- **範囲:** JRA（札幌・函館・福島・新潟・東京・中山・中京・京都・阪神・小倉）のみ
- **提示:** レース単位・当日横断・券種フィルタの3軸
...
- ワイド/馬連: 買い目2頭の `placePotential` の合成（初期は平均 or 下限）
- 3連系: 3頭の合成（初期は平均 or 下限）。将来変更可
...
- 過去の JRA レースで「当該券種オッズ ≥ 閾値 かつ的中」を正解集合とする
- 再現率・適合率・候補件数を計測し、閾値とスコア実装を調整／差し替え
...
1. ヒーロー（ブランド）
2. **今日の注目穴**（設定閾値以上・スコア上位 N 件、券種混在可）
3. レース一覧への導線
4. 選別ロジックの要約（JRA限定・全券種・閾値可変）
...
type Horse = {
  ...
  placePotential: number; // 現行スコアラーの出力（仮でも可）
  factors: {
    courseFit: number;
    paceFit: number;
    conditionFit: number;
...
- スコアは `Scorer` 経由でのみ算出し、実装差し替えに耐える
...
src/
  ...
  domain/
    ...
    scoring/            # Scorer IF + ruleBased（差し替え可）
    betTypes.ts         # 券種定義
...
1. データモデルを JRA + 全券種オッズ対応に拡張  
2. `Scorer` IF とルール仮スコア、`selectLongshots` を追加  
3. `/settings` で閾値変更可能にする  
4. `/longshots` をメイン画面として実装  
5. トップ／レース詳細を穴選別導線に合わせて更新  
