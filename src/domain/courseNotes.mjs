 ...
 * 新潟芝は形状の注記のみ（前残りは従来どおり全距離。枠は載せない）。
 * 新潟ダ1200は芝スタートの外枠・前残りをスコアへ。京都は未検証。
 * UI の説明と、枠バイアスの小さな補正の単一ソース。
 * courseFit / paceFit には載せない（馬ごとの同条件・脚質データではないため）。
 * 場を瞬発戦／持続力戦に分類しない。コースのラップ二分法はスコアにも載せない。
 * 前走そのレースのペースで着順を読み替えるのは formSignal 側。
...
const HANSHIN = {
  generic: {
    summary: "右回り。内回りと外回りで別物。ゴール前に急坂。2025改修後の馬場は別物。",
    bullets: [
...
const NAKAYAMA = {
  generic: {
    ...
    bullets: [
      ...
      "ゴール前は高低差2.2m・最大勾配2.24%の急坂。直線のスピードだけでは押し切れない。",
...
/** 形状の注記のみ。枠・前残りスコアには載せない。 */
const NIIGATA = {
  generic: {
    summary: "左回り。芝外の直線658.7mは全場最長。ダ1200のみ芝スタート。",
    bullets: [
      "左回りは東京・中京とこの3場だけ。",
      "芝1800は外回り、芝2000は内回りで別物。距離が近いだけでスタート位置の延長ではない。",
      "直線は平坦に近い。芝の前残りは従来どおり全距離（危1の補助）。枠スコアへは載せない。",
      "ダート1200は芝スタートで外枠・前をスコアへ。ほかの距離はダート発走。",
    ],
    gate: { mode: "none" },
    frontBias: false,
    scoreInGate: false,
  ...
  "ダート:1200": {
    summary: "芝スタート。外枠・前。",
    bullets: [
      "芝スタート。2角ポケットから芝を踏んでダートへ。外側ほど芝が長い。",
      "外枠の逃げ・先行が残りやすい。内は芝が短く砂を被りやすい。",
      "東京ダ1600・阪神ダ1400と同じ厚みの外。京都ダ1400は未検証。",
    ],
    gate: { mode: "outer", boost: 2 },
    frontBias: true,
    scoreInGate: true,
...
/** 新潟芝は従来どおり全距離。新潟ダ1200は芝スタートの前残り。ほかは検証済み距離の frontBias だけ。京都は未検証。 */
